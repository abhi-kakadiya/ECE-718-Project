...
Jointly optimizes both instructions and few-shot demonstrations using
...
class MIPROv2Optimizer:
    ...
    This optimizer:
    1. Bootstraps demonstration candidates
    2. Proposes multiple instruction variants (data-aware)
    3. Uses Bayesian optimization to find best combination
    4. Returns optimized program with instructions + demos
    """
    ...
    def __init__(self,
        ...
        self.token_tracker = TokenTracker(benchmark_name, "miprov2")

        self.optimized_module = None
        self.compilation_time = 0.0

        metric_type = "performance-aware" if use_performance_metric else "quality-only"
        logger.info(f"Initialized MIPROv2Optimizer for {benchmark_name}")
        logger.info(f"  Instruction candidates: {num_candidates}")
        logger.info(f"  Init temperature: {init_temperature}")
        logger.info(f"  Metric type: {metric_type} (weight={performance_weight if use_performance_metric else 'N/A'})")

    def compile(self,
                trainset: List[dspy.Example] = None,
                num_trials: int = 10) -> 'MIPROv2Optimizer':
        """
        Compile the optimizer using MIPROv2.
        ...
        logger.info(
            f"\n{'='*80}\n"
            f"Compiling MIPROv2 optimizer for {self.benchmark_name}\n"
            f"{'='*80}\n"
        )

        # Load training examples if not provided
        if trainset is None:
            logger.info("Loading default training examples...")
        ...
        logger.info("Creating MIPROv2 teleprompter...")
        ...
        # Compile the module
        logger.info("Compiling module (this may take several minutes)...")
        logger.info("  Phase 1: Bootstrapping demonstration candidates")
        logger.info("  Phase 2: Proposing instruction variants")
        logger.info("  Phase 3: Bayesian optimization")
        logger.info("  Phase 4: Selecting best configuration")

        try:
            # Add minibatch parameter to avoid valset size errors with small training sets
            self.optimized_module = teleprompter.compile(
                student=self.module,
                trainset=trainset,
                num_trials=num_trials,
                minibatch_size=2,  # Smaller batch size for small training sets
            ...
            if hasattr(dspy.settings, 'lm') and hasattr(dspy.settings.lm, 'history'):
                ...
                # Clear history for inference
                dspy.settings.lm.history.clear()

            logger.info("✓ MIPROv2 optimization successful!")
    ...
    def generate_code(self, **kwargs) -> Dict[str, Any]:
        """
        Generate code using MIPROv2-optimized module.
        ...
        logger.debug("Generating code with MIPROv2-optimized module")
...
def run_miprov2_optimizer(
    ...
    """
    Run the MIPROv2 optimizer for a benchmark.
    ...
    logger.info(
        f"\n{'='*80}\n"
        f"Running MIPROV2 optimizer: {benchmark_name} (size={size})\n"
        f"{'='*80}\n"
    )
    ...
    optimizer.compile(trainset=trainset, num_trials=num_trials)
    ...
    logger.info(f"\nMIPROv2 optimizer completed for {benchmark_name}\n")
